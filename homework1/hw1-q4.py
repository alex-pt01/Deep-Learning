...
class FeedforwardNetwork(nn.Module):
    def __init__(
        ...
        # === always drops-out in first layer
        self.layers.append(nn.Dropout(dropout))
        if(layers==1):
            self.layers.append(nn.Linear(n_features, n_classes))
            self.layers.append(nn.Tanh() if activation_type=='tanh' else nn.ReLU())
        else:
            # === First Layer ===
...
def main():
    ...
    print("n_feats: "+ str(n_feats) + "\nn_classes: " + str(n_classes))

    # initialize the model
    if opt.model == 'logistic_regression':
        model = LogisticRegression(n_feats, n_classes)
    else:
